 ...
 * All Rights Reserved.
 *
 * FILE:        $Source: /bmrb/cvs_archive/cvs/sansj/doc/examples/Time2.java,v $
 * 
 ...
 * @version 1
 */
public class Time2 implements ContentHandler, ErrorHandler {
...
// read until we hit SFNAME
	    sans.parse( true );
// backup save_ + SFNAME.length() characters
            sans.backup( 20 );
// write Saveframe to a StringWriter
	    StringWriter sw = new StringWriter(); 
	    sans.write( sw, SansParserConstants.SAVEEND );
// create a StringReader
	    StringReader sr = new StringReader( sw.toString() );
//  and a StarParser
	    StarParser star = new StarParser( sr );
// parse in the saveframe
	    star.SaveFrameNodeParse( star );
// print it out
	    StarUnparser unparser = new StarUnparser( System.out );
	    unparser.writeOut( (SaveFrameNode)star.endResult(), 1 );
	    
	    System.out.println( "SANS + Starlib: " + (System.currentTimeMillis() - start) );
	    in = null;
	    ...
	    StarFileNode node = (StarFileNode)star.endResult();
	    VectorCheckType sf = node.searchByName( "save_" + SFNAME );
	    if( sf.size() > 0 )
		unparser.writeOut( (SaveFrameNode)sf.elementAt( 0 ), 1 );
